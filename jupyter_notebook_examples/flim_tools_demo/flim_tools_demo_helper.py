...
from skimage.filters import threshold_otsu, threshold_yen, threshold_minimum, threshold_multiotsu
...
def remove_artificats_demo(image, show_plots = True, re = False):
    ...
    cleared_max = np.maximum(np.array(cleared_otsu).astype(int), np.array(cleared_min).astype(int))
...
def label_and_assign_rois(image, show_plots = True, re = False):
    cleared_otsu, cleared_yen, cleared_min, cleared_max = remove_artificats_demo(image, show_plots=False, re=True)
    ...
    if show_plots:
        ...
        labeled_thresholding_methods = [label_image_otsu, label_image_yen, label_image_min, label_image_max]

        for index, labeled_thresholding_image in enumerate(labeled_thresholding_methods): 
...
def full_pipeline(image, show_plots = True, re = False): 
    label_image_otsu, label_image_yen, label_image_min, label_image_max = label_and_assign_rois(image, show_plots=False, re=True)
    print
    edges = canny(
        image=label_image_max,
        sigma=2.0,
        low_threshold=0.01,
        high_threshold=0.1,
        )
    ...
    nb_components, output, stats, centroids = cv2.connectedComponentsWithStats(binary_filled_edges.astype(np.uint8), connectivity=8)
