...
def regionprops_omi_example(path_output):
    ...
    list_all_nadh_photons_images = list(filter(re.compile(r".*n_photons.asc").search, list_str_all_files ))
    ...
    for path_str_im_photons in tqdm(list_all_nadh_photons_images):
        ...
        # paths to files
        dict_dir[handle_im]["nadh_photons"] = list(filter(re.compile(handle_nadh +  file_suffixes['im_photons']).search, list_str_all_files))[0]
        ...
        # MASKS
        try:
            dict_dir[handle_im]["mask_cell"] = list(filter(re.compile(handle_nadh +  file_suffixes['mask_cell']).search, list_str_all_files))[0]
            dict_dir[handle_im]["mask_cytoplasm"] = list(filter(re.compile(handle_nadh +  file_suffixes['mask_cytoplasm']).search, list_str_all_files))[0]
            dict_dir[handle_im]["mask_nuclei"] = list(filter(re.compile(handle_nadh +  file_suffixes['mask_nuclei']).search, list_str_all_files))[0]
        except IndexError:
        ...
        try:
            path_str_im_photons_fad = list(filter(re.compile(handle_im + "f" + file_suffixes["im_photons"]).search, list_str_all_files))[0]
        except IndexError:
        ...
        # paths to images
        dict_dir[handle_im]["fad_photons"] = list(filter(re.compile(handle_fad +  file_suffixes['im_photons']).search, list_str_all_files))[0]
...
if __name__ == "__main__":
    ...
    path_output = Path(r"C:\Users\econtrerasguzman\Desktop\development\flim_tools\examples") 
    ...
    for key, item in list(df.iterrows())[:1]:
        ...
        if np.sum(rr_means.values < rr_thresh_low) + np.sum(rr_means > rr_thresh_high) > 0:
