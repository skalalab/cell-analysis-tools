...
F1 = full_data[full_data.columns[0:12]] #this subselects columns 1-13 into a new dataframe so just the numeric data will be fed in (from FADA1 to Redox Ratio)
...
print("Accuracy on the test set with raw data: {:.5f}".format(clfLR.score(X_test, y_test))
)
...
working_directory = Path(os.getcwd())
print(working_directory)
path_data = working_directory / "/Users/danbrownie/Desktop/Tiffany_scripts/master sheet 0120.csv"
full_data = pd.read_csv(path_data, header=0) #reads in data for Efficiency classification
...
y = full_data[full_data.columns[15]] #class labels by Type (T cell - 0, Tumor cell - 1)
...
#Determines scaling factors to make data range 0 - 1 (min value assigned 0, max value assigned 1)
std_scale = preprocessing.MinMaxScaler().fit(F1[['n.p.mean',
'n.t1.mean', 'n.t2.mean', 'n.a1.mean',
'n.a2.mean', 'f.p.mean', 'f.t1.mean',
'f.t2.mean', 'f.a1.mean', 'f.a2.mean',
'rr.mean', 'n.tm.mean','f.tm.mean']])

#Transforms the numeric data with the scaling factors determined above
df_std = std_scale.transform(F1[['n.p.mean',
'n.t1.mean', 'n.t2.mean', 'n.a1.mean',
'n.a2.mean', 'f.p.mean', 'f.t1.mean',
'f.t2.mean', 'f.a1.mean', 'f.a2.mean',
'rr.mean', 'n.tm.mean','f.tm.mean']])
...
for feature in features:
    ...
    this_scoresSVM = cross_val_score(clfSVM,df_std, y, cv = 15) #Estimates the Efficiency classification accuracy score based on best features used, with 15-fold cross validation 
...
plt.axis('tight')
plt.savefig('Accuracy_SVM_byvar_CAR.eps') #Save .eps file of graph (can change to tif or other file type)
plt.show()
...
for feature in features:
    ...
    this_scoresRF = cross_val_score(clfRF,df_std, y, cv = 15) #Estimates the Efficiency classification accuracy score based on best features used, with 15-fold cross validation
...
plt.ylabel('Accuracy Score')
plt.axis('tight')
plt.savefig('Accuracy_RF_byvar_CAR.eps') #Save .eps file of graph (can change to tif or other file
plt.show()
...
plt.legend(loc="best")
plt.title("ROC curve")
plt.savefig('LR_RF_ROCcurve_CAR.eps') #Save .eps file of graph (can change to tif or other file
plt.show()
...
for feature in features:
    ...
    lrc_this_scores = cross_val_score(lrc_clf,df_std, y, cv = 15) #Estimates the Efficiency classification accuracy score based on best features used, with 15-fold cross validatio
...
plt.ylabel('Accuracy Score')
plt.axis('tight')
plt.savefig('Accuracy_LR_byvar_CAR.eps') #Save .eps file of graph (can change to tif or other file
plt.show()
plt.show()
...
# TODO check C value
lrcNTM = LogisticRegression(C= 1438.44988828766, penalty = 'l2', solver = 'newton-cg', random_state=42) #Check C value?
...
#Train LR Classifier with just NADH tm and FAD tm
# TODO check C value
lrcNTMFTMtest = LogisticRegression(C= 1438.44988828766, penalty = 'l2', solver = 'newton-cg', random_state=42)
...
lrc_FAD = LogisticRegression(C= 1438.44988828766, penalty = 'l2', solver = 'newton-cg', random_state=42)
...
lrc_NADH = LogisticRegression(C= 1438.44988828766, penalty = 'l2', solver = 'newton-cg', random_state=42)
