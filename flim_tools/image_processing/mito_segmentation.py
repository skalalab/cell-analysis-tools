...
if __name__ == "__main__":
    ...
    # load image 
    path_images = Path(r"Z:\0-Projects and Experiments\TQ - AAG - mitochondria-actin segmentation\210205_GoldStandard_NADH_images".replace("\\",'/'))
    ...
    # load mask
    path_masks = Path(r"Z:\0-Projects and Experiments\TQ - AAG - mitochondria-actin segmentation\210111_GoldStandard_Stain_Masks".replace('\\','/'))
    ...
    # mask
    path_mask_aag =  list(filter(re.compile(f"{handle_tmre}").search, list_path_masks_aag_str))[0]
    path_mask_tq = list(filter(re.compile(f"{handle_tmre}").search, list_path_masks_tq_str))[0]
...
#%%    
    ...
    for pos, params in enumerate(parameters, start=1):
        ...
        assert not (top_hat - im_subtraction).any(), "top_hat does not match manual top hat"
        
        
        if im_subtraction.sum() == 0:
            print(f"top_hat returned all zeros: bin_size:{bin_size} radius: {selem_radius}")
            continue
        
        #cluster
        k=2
        n_brightest_clusters=1
        im_kmeans = kmeans_threshold(top_hat, k=k, n_brightest_clusters=n_brightest_clusters)
        ...
        if debug:
            ...
            fig.suptitle(f"bin: {kernel_size}x{kernel_size}  disk_radius:{selem_radius}")
