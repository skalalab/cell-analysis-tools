...
def pad_to_square(im, debug=False):
    ...
    if pad == "rows":
        pass
        im_square = np.pad(
            im, ((before, after), (0, 0)), mode="constant", constant_values=0
        )
    if pad == "cols":
        pass
        im_square = np.pad(
            im, ((0, 0), (before, after)), mode="constant", constant_values=0
        )
...
def fractal_dimension_gray(roi, intensity, mode="standard", debug=False):
    ...
    if debug:
        ...
        plt.figure(1, figsize=(10, 5)).canvas.set_window_title(
