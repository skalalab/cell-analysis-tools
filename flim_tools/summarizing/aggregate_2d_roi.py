# -*- coding: utf-8 -*-
"""
Created on Fri Oct 23 11:10:07 2020

@author: Nabiki/Emmanuel Contreras Guzman

This script exports per roi decays and spcm data csv summaries
inputs are the spcm tiff files and the sdt files 
"""
...
def aggregate_sdt_roi(mask, sdt_cube, debug=False):
    ...
    list_roi_values = np.delete(list_roi_values, np.where(list_roi_values == 0)) # exclude bg
    
    # store decays
    list_decays = []
    # ITERATE THROUGH ROI's
    for roi_idx, roi_value in enumerate(list_roi_values):
        if debug: print(f"processing roi {roi_idx+1}/{len(list_roi_values)}")
        pass
        ...
        #################
        # AGGREGATE DECAY
        masked_sdt_data = im_sdt * m_roi[...,np.newaxis] # mask out rest of pixels/decays
...
for path_dish in list_paths_dishes[:5]:# list_paths_dishes[0:1] # oly select these dishes with valid info
    pass
    ...
    for pos, path_sdt in enumerate(list_path_sdt):
        ...
        path_photons = list(filter(re.compile(pattern).match,list_path_all_files)) # returns list of matching files, should be 1
        if not _validate_file_found(path_photons): 
            print("photons")
            continue
        else : path_photons = Path(path_photons[0]) # one item list, get string 
        ...
        if not _validate_file_found(path_a1): 
            print("a1")
            continue
        else : path_a1 = Path(path_a1[0]) # one item list, get string 
        ...
        if not _validate_file_found(path_a2): 
            print("a2")
            continue
        else : path_a2 = Path(path_a2[0]) # one item list, get string 
        ...
        if not _validate_file_found(path_t1):
            print("t1")
            continue
        else : path_t1 = Path(path_t1[0]) # one item list, get string 
        ...
        if not _validate_file_found(path_t2):
            print("t2")
            continue
        else : path_t2 = Path(path_t2[0]) # one item list, get string 
        ...
        if not _validate_file_found(path_chisq): 
            print("chisq")
            continue
        else : path_chisq = Path(path_chisq[0]) # one item list, get string 
        ...
        plt.title(f"{path_sdt.parent.stem} - {prefix} \n  sdt  | mask_whole_cell  | mask_cytoplasm")
...
for idx, im_set in enumerate(dataset): # look at first image, last in set is not good
    ...
    for roi_idx, roi_value in enumerate(list_roi_values):
        ...
        list_regionprop_header = ['area', 'major_axis_length', 'minor_axis_length', 'perimeter']
        ...
        if not df_features_header:
            df_features_header = ["image_roi_id"] + list_roi_features_header + list_regionprop_header
