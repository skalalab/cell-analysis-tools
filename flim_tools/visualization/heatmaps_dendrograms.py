...
path_data = (
    working_directory / "Final_Full_MeanintensitiesEPCiPSCMid_USEDFORUMAP_Patterned.csv"
)
...
df_metabolic_features = pd.DataFrame(
    df_metabolic_features.values, columns=metabolic_headers
)
...
plt.savefig(
    working_directory / "all_data.png", dpi=400, format="png", bbox_inches="tight"
)
...
plt.title(f"mean intensities")
plt.savefig(
    working_directory / "heatmap_by_donor_number.png",
    dpi=400,
    format="png",
    bbox_inches="tight",
)
...
# plot finally #z_score=0
g = sns.clustermap(
    df_class_features, standard_scale=1, cmap="viridis", figsize=(10, 10)
)

# other plot parameters
plt.title(f"mean intensities")
plt.savefig(
    working_directory / "heatmap_by_class.png",
    dpi=400,
    format="png",
    bbox_inches="tight",
)
